...
namespace BI.STQA.Utils
{
    ...
    internal class ApprovalMailUtil
    {
        ...
        internal static void SendAbordMail(List<string> mailList, TET_SupplierSTQA dbModel, string userID, DateTime cDate)
        {
            EMailContent content = new EMailContent()
            {
                Title = $"[審核中止通知] 新增STQA資料審核_{dbModel.BelongTo}",
                Body =
                $@"
                您好，<br/>
                <br/>
        ...
        /// <param name="userID">目前登入者</param>
        /// <param name="cDate">目前時間</param>
        internal static void SendNewApprovalMail(string receiverMail, TET_SupplierSTQAApproval approvalModel, TET_SupplierSTQA dbModel, string userID, DateTime cDate)
        {
            var pageUrl = $"{ModuleConfig.EmailRootUrl}/SupplierApproval/Index";

            EMailContent content = new EMailContent()
            {
                Title = $"[審核通知] 新增STQA資料審核_{dbModel.BelongTo}",
                Body =
                $@"
您好,<br/>
