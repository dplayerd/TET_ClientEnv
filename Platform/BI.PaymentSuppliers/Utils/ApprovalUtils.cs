...
namespace BI.PaymentSuppliers.Utils
{
    public class ApprovalUtils
    {
        #region ApprovalLevel
        /// <summary> ApprovalLevel 轉換錯誤訊息 </summary>
        public static string ParseApprovalLevelError = $"Level is required, and must be ['{ApprovalLevel.User_GL.ToText()}', '{ApprovalLevel.ACC_First.ToText()}', {ApprovalLevel.ACC_Second.ToText()}', '{ApprovalLevel.ACC_Last.ToText()}']";
        ...
        public static ApprovalType ParseApprovalType(string val)
        {
            ...
            ApprovalType enm;
            if (val == ApprovalType.New.ToText())                // 新增付款單位審核
                enm = ApprovalType.New;
            else if (val == ApprovalType.Modify.ToText())        // 付款單位改版審核
