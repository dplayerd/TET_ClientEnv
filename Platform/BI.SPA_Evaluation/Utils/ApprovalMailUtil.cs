...
{reason.ReplaceNewLine(true)}
        ...
        /// <param name="userID">目前登入者</param>
        /// <param name="cDate">目前時間</param> 
        internal static void SendNewVerifyMail(UserAccountModel receiver, TET_SPA_CostServiceApproval approvalModel, TET_SPA_CostService main, string userID, DateTime cDate)
        {
            var pageUrl = $"{ModuleConfig.EmailRootUrl}/SupplierApproval/Index";

            EMailContent content = new EMailContent()
            {
                Title = $"[Cost&Service資料審核] {main.Period}_{receiver.FirstNameEN} {receiver.LastNameEN} ({receiver.EmpID})",
                Body =
                $@"
您好,<br/>
